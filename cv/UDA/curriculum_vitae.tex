...
\begin{document}
	...
	\begin{innerlist}
		...
		\item Familiar with Python (\emph{CasADi, Pandas, Numpy, Scipy}), bash, version control (\emph{Git}) and unit-testing.
	\end{innerlist}
	...
	\textbf{Computing} Matlab/Simulink, C/C++, Python
	
	\vspace{0.1cm}
	
	\textbf{IT} Python (\emph{Flask, BeautifulSoup, Tkinter}), JS (\emph{Highcharts}), Heroku (\emph{Postgres}), AWS (\emph{RDS}), Hugo, API
	
	\vspace{0.1cm}
	
	\textbf{Hardware} PLC (\emph{Siemens, Schneider, Omron}), ARM (\emph{STM32F7/L4}), Raspberry Pi
	
	\vspace{0.1cm}
	
	\textbf{Editing} MS Office, TeX, Markdown, Inkscape, Webslides, AutoCAD
